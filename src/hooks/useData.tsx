...
import {County, GeoJSONData, Judge, NewYorkState, SortTarget} from '@/types/frontendTypes';
import {getCounties, getGeoJson, getJudgeImportances, getJudges, getNewYorkState} from "./api.ts";
...
type DataContextType = {
    ...
    setSelectedEntity: React.Dispatch<React.SetStateAction<Judge | County | null>>;
    deviation: boolean;
    ...
    setCountyNameFilter: React.Dispatch<React.SetStateAction<string>>;
...
export const DataProvider: React.FC<DataProviderProps> = ({children}) => {
    ...
    const [selectedEntity, setSelectedEntity] = useState<Judge | County | null>(null);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [newYorkGeoJson, fetchedCounties, fetchedJudges, fetchedNewYorkState] = await Promise.all([
                    getGeoJson(),
                    getCounties(),
                    getJudges(),
                    getNewYorkState()
                ...
                setAllCounties(fetchedCounties.sort((a, b) => a.name.localeCompare(b.name)));
    ...
    useEffect(() => {
        const fetchJudgeImportances = async () => {
            if (selectedJudge) {
                try {
                    const bailImportances = await getJudgeImportances(selectedJudge.name, 'bail');
                    const remandImportances = await getJudgeImportances(selectedJudge.name, 'remand');
