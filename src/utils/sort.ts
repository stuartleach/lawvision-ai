...
export const sortListByTargetGivenRaceAndSeverity = (
  ...
  const list = listRaw.filter(
    (item) => item.arraignmentResults[severity].Any.totalCases >= minCases,
  );
  ...
  const getDeviationFromAverageRace = (item: Judge | County): number | null => {
    ...
    switch (target) {
      ...
      case SortTarget.remanded:
        return (
          (targetData?.remanded?.percent || 0) -
          (avgData?.remanded?.percent || 0)
        );
      case SortTarget.released:
        return (
          (targetData?.released?.percent || 0) -
          (avgData?.released?.percent || 0)
        );
      case SortTarget.bailSet:
        return (
          (targetData?.bailSet?.percent || 0) - (avgData?.bailSet?.percent || 0)
        );
      case SortTarget.name:
  ...
  const compareValues = (a: Judge | County, b: Judge | County): number => {
    const aVal =
      deviation && race !== "Any"
        ? getDeviationFromAverageRace(a)
        : getMetricValue(a);
    const bVal =
      deviation && race !== "Any"
        ? getDeviationFromAverageRace(b)
        : getMetricValue(b);
