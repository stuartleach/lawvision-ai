...
import { Judge } from "../types/frontendTypes";

interface JudgeInfoHeaderProps {
  entity: Judge | null;
}

const JudgeInfoHeader: React.FC<JudgeInfoHeaderProps> = ({ entity }) => {
  return (
    <div className="flex sm:flex-row flex-col justify-between py-2">
      ...
      {entity?.primaryCounty && (
        <h2 className="font-semibold py-4 sm:mt-0 font-sans hidden sm:block text-4xl text-center sm:text-left sm:text-4xl tracking-tight text-gray-200 pr-5">
          {entity ? " Trial Judge in " : ""}{" "}
          <span className="bg-gradient-to-tr from-blue-500 to-blue-300 bg-clip-text font-bold text-transparent">
            {entity.primaryCounty} County
          </span>
        </h2>
      )}
    </div>
...
export default JudgeInfoHeader;
