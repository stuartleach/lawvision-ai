...
import {ArraignmentResultsByRace, County, Judge} from '../types/frontendTypes';
...
const ProfileVisualizations: React.FC<ProfileVisualizationsProps> = ({entity}) => {
    const {graphTargetData, allCounties, newYorkState, selectedJudge} = useData();
    ...
    const judgeStats = getAllRaceStatsForMetric(judge?.arraignmentResults?.[severity], metric, val);
    const countyStats = getAllRaceStatsForMetric(county?.arraignmentResults?.[severity], metric, val);
    const stateStats = getAllRaceStatsForMetric(state?.arraignmentResults?.[severity], metric, val);
            ...
            <div className="w-[40rem] h-96">
                {metric === 'bailSet' && val === 'amount' ? (
                    <div className="text-center tracking-tight justify-center text-zinc-400 text-xl mb-8">
                        <b className="text-zinc-300">Average bail set</b> in <b
                        className="text-zinc-300">{severity === 'Any' ? 'all' : severityLabels[severity]}</b> cases.
                    </div>
                ) : (
                    <div className="text-center tracking-tight justify-center text-zinc-400 text-xl mb-8">
                        Percent of <b
                        className="text-zinc-300">{severity === 'Any' ? 'all' : severityLabels[severity]}</b> cases
                        where
                        <b className="text-zinc-300">{race !== 'Any' ? 'defendant is ' + race + ' and ' : ''}</b>
