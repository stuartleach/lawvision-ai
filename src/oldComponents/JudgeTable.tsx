...
const JudgeTable: React.FC = () => {
    ...
    useEffect(() => {
        let judges = allJudges.filter(judge => judge.arraignmentResults.Any.Any.totalCases > 9);
        if (countyFilter) {
            judges = judges.filter(judge => judge.primaryCounty === countyFilter);
        }
        if (nameFilter) {
            judges = judges.filter(judge => judge.name.toLowerCase().includes(nameFilter.toLowerCase()));
        }
        const sortedJudges = sortListByTargetGivenRaceAndSeverity(judges, sortTarget, sortOrder);
