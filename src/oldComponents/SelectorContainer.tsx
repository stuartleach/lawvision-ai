...
const SelectorContainer: React.FC = () => {
    ...
    const [judges, setJudges] = useState(allJudges);
    const [counties, setCounties] = useState(allCounties.sort((a, b) => a.name.localeCompare(b.name)));
    const [severityOptions] = useState(['Any', 'AF', 'BF', 'CF', 'DF', 'EF', 'AM', 'BM', 'I', 'V']);
    const [localSortOrder, setLocalSortOrder] = useState<SortOrder>(SortOrder.desc);
    const [currentListTarget, setCurrentListTarget] = useState<'judges' | 'counties'>('judges');
    ...
    const items = {
        judges: {
            name: 'Judges',
            targets: countyNameFilter ? judges.filter((c) => c.primaryCounty === countyNameFilter) : judges,
        },
    ...
    const handleNextSortDirection = () => {
        setLocalSortOrder(prevOrder => prevOrder === SortOrder.desc ? SortOrder.asc : SortOrder.desc);
    };
                        ...
                        <section className="flex flex-col gap-1 text-right">
                            <button onClick={handleNextSort}>
                                <p className="text-zinc-500 text-sm">
                                    sorted by
                                    <span className={`text-left text-sm tracking-[-.04em] bg-gradient-to-tr from-blue-500 to-blue-300 bg-clip-text font-bold text-transparent ${sortTargetColor(sortTarget)}-color`}>
                            ...
                            <button onClick={handleNextSortDirection}>
                                <p className="text-zinc-500 text-sm">
                                    direction
                                    <span className={`text-left text-sm tracking-[-.04em] bg-gradient-to-tr from-blue-500 to-blue-300 bg-clip-text font-bold text-transparent ${sortTargetColor(sortTarget)}-color`}>
                                        {localSortOrder === SortOrder.asc ? 'Lowest to highest' : 'Highest to lowest'}
                                    </span>
                                </p>
                            </button>
                        </section>
            ...
            <ul role="list" className="divide-y divide-zinc-800/30">
                {entityList.length > 0 ? (
                    ...
                    <li className="gap-x-4 py-5 cursor-pointer p-6 flex flex-row justify-start">
                        ...
                        <button className="flex text-zinc-300 hover:text-zinc-100 transition" onClick={clearAllFilters}>
                            Clear Filters
                        </button>
                    </li>
                )}
