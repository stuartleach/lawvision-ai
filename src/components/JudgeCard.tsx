...
export const JudgeCard: React.FC = () => {
    ...
    useEffect(() => {
        if (selectedJudge?.primaryCounty) {
            const countyObject = allCounties.find(c => c.name === selectedJudge.primaryCounty);
            if (countyObject) {
...
const JudgeProfile: React.FC = () => {
    ...
    const judgeCounty = allCounties.find(county => county.name === selectedJudge.primaryCounty);
