...
const FeatureImportanceChart: React.FC = () => {
    ...
    const {selectedJudge, selectedCounty, setSelectedCounty, newYorkState, allCounties} = useData();

    // Function to transform featureImportances data for the chart
    const transformFeatureImportances = (featureImportances: FeatureImportances | undefined) => {
        if (!featureImportances) return [];

        return Object.entries(featureImportances)
            .filter(([key, value]) => key !== 'judge' && key !== 'case_count' && typeof value === 'number')
    ...
    const featureImportances: Record<DecisionType, any[]> = { // Replace 'any' with the actual type of the transformed data
        bailAmount: transformFeatureImportances(selectedJudge?.featureImportancesBailSet),
    ...
    return (
        <Card className="bg-zinc-900 text-zinc-300 border-zinc-800 flex flex-col justify-center *:justify-center">
            <CardHeader>
                <CardTitle>Comparative Feature Importance in Pretrial Decisions</CardTitle>
                <CardDescription className="text-zinc-400">
                    Relative importance of factors at Judge, County, and State levels
                </CardDescription>
            </CardHeader>
            <CardContent>
                <Tabs value={decisionType} onValueChange={(value) => setDecisionType(value as DecisionType)}>
