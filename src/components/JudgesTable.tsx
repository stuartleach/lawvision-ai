...
import {Judge, Race, ResultsBySeverity, SeverityLevel} from '@/types/frontendTypes.ts';
...
const JudgesTable: React.FC = () => {
    const {allJudges, setSelectedJudge, judgeNameFilter, newYorkState, allCounties} = useData();
    ...
    useEffect(() => {
        const max = Math.max(...allJudges.map(judge => judge.arraignmentResults.Any.Any.totalCases));
        setMaxCases(max);
        setMinCases(0);
    }, [allJudges]);

    const getFieldValue = (arraignmentResults: ResultsBySeverity, field: SortField): number => {
        switch (field) {
            case 'totalCases':
    ...
    const getStateAverage = useMemo(() => (field: SortField): number => {
    ...
    const getCountyAverage = useMemo(() => (field: SortField, ju: Judge): number => {
    ...
    useEffect(() => {
        ...
        sorted.sort((a, b) => {
            ...
            switch (sortField) {
                ...
                case 'releasedPercent':
                case 'jailPercent':
                    comparison = getFieldValue(a.arraignmentResults, sortField) - getFieldValue(b.arraignmentResults, sortField);
                    break;
            }
    ...
    }, [allJudges, judgeNameFilter, sortField, sortDirection, race, severity, minCases, county]);
    ...
    const SortIcon = ({field}: { field: SortField }) => {
        ...
        return sortDirection === 'asc' ? <ChevronUpIcon className="w-4 h-4 inline"/> :
    ...
    const renderCellValue = (judge: Judge, field: SortField) => {
        ...
        if (compareToStateAverage) {
            const differenceColor = stateDifference > 0 ? 'text-green-400' : 'text-red-400';
            return (
                <span>
                    {formattedValue}
                    <span className={`ml-2 ${differenceColor}`}>
                        ({statePercentDifference > 0 ? '+' : ''}{formatPercent(statePercentDifference)}%)
                    </span>
                </span>
            );
        } else if (compareToCountyAverage) {
            const differenceColor = countyDifference > 0 ? 'text-green-400' : 'text-red-400';
            return (
                <span>
                    {formattedValue}
                    <span className={`ml-2 ${differenceColor}`}>
                        ({countyPercentDifference > 0 ? '+' : ''}{formatPercent(countyPercentDifference)}%)
                    </span>
                </span>
            );
        }
            ...
            <Table>
                <TableHead>
                    <TableRow>
                        ...
                        <TableHeader onClick={() => handleSort('primaryCounty')} className="cursor-pointer">
                            Primary County <SortIcon field="primaryCounty"/>
                        </TableHeader>
                        <TableHeader onClick={() => handleSort('totalCases')} className="cursor-pointer">
                            Total Cases <SortIcon field="totalCases"/>
                        </TableHeader>
                        <TableHeader onClick={() => handleSort('bailSetPercent')} className="cursor-pointer">
                            % Bail Set <SortIcon field="bailSetPercent"/>
                        </TableHeader>
                        <TableHeader onClick={() => handleSort('averageBail')} className="cursor-pointer">
                            Average Bail $ <SortIcon field="averageBail"/>
                        </TableHeader>
                        <TableHeader onClick={() => handleSort('releasedPercent')} className="cursor-pointer">
                            % Released <SortIcon field="releasedPercent"/>
                        </TableHeader>
                        <TableHeader onClick={() => handleSort('jailPercent')} className="cursor-pointer">
