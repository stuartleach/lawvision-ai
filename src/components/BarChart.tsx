...
import {ArraignmentResults, County, Judge, NewYorkState, Race, SeverityLevel} from "@/types/frontendTypes.ts"
...
const JudgeComparisonChart: React.FC<JudgeComparisonChartProps> = ({judge, county, state}) => {
        ...
        <Card className="bg-zinc-900 text-zinc-300 border-zinc-800">
            ...
            <CardContent>
                <div className="mb-4 flex gap-4">
                    <Select onValueChange={(value) => setSeverity(value as SeverityLevel)} defaultValue={severity}>
                        ...
                        <SelectContent className="bg-zinc-800 border-zinc-700">
                            {Object.keys(judge.arraignmentResults).map((sev) => (
                                <SelectItem className="text-zinc-300" key={sev} value={sev}>{sev}</SelectItem>
                            ))}
                        </SelectContent>
                    </Select>
                    <Select onValueChange={(value) => setRace(value as Race)} defaultValue={race}>
                        <SelectTrigger className="bg-zinc-800 text-zinc-300 border-zinc-700">
                            <SelectValue placeholder="Select race"/>
                        </SelectTrigger>
                        <SelectContent className="bg-zinc-800 border-zinc-700">
                            {Object.keys(judge.arraignmentResults.Any).map((r) => (
                                <SelectItem className="text-zinc-300" key={r} value={r}>{r}</SelectItem>
                            ))}
                        </SelectContent>
                    </Select>
                </div>
                <div className="w-full aspect-square max-h-[400px]">
                    <RadarChart outerRadius="80%" width={400} height={400} data={chartData}>
                        ...
                        <PolarAngleAxis dataKey="metric" tickFormatter={formatTick} stroke="#9CA3AF"/>
                        <Radar name="Judge" dataKey="Judge" stroke="#60A5FA" fill="#60A5FA" fillOpacity={0.2}/>
                        <Radar name="County" dataKey="County" stroke="#34D399" fill="#34D399" fillOpacity={0.2}/>
                        <Radar name="State" dataKey="State" stroke="#FBBF24" fill="#FBBF24" fillOpacity={0.2}/>
            ...
            <CardFooter className="flex-col gap-2 pt-4 text-sm">
                <div className="flex items-center gap-2 leading-none text-zinc-400">
                    Pretrial decisions when the charge is of severity <span
                    className="font-mono bg-zinc-800 p-1 rounded">{severity}</span> and the defendant's race is <span
                    className="font-mono bg-zinc-800 p-1 rounded">{race.toLowerCase()}</span>.
                </div>
            </CardFooter>
        </Card>
    );
...
export function ShadBarChart({judge, county, state}: Props) {
    const [metric, setMetric] = useState<'% Bail Set' | 'Average Bail $' | '% Released' | '% Jailed'>('% Bail Set')
    const [severity, setSeverity] = useState<SeverityLevel>('Any')
    ...
    const createDataPoint = (entity: 'Judge' | 'County' | 'State', data: Record<Race, ArraignmentResults>): ChartDataPoint => ({
        entity,
        White: getMetricValue(data.White),
        Black: getMetricValue(data.Black),
        'American Indian/Alaskan Native': getMetricValue(data['American Indian/Alaskan Native']),
    ...
    const races: Race[] = ['White', 'Black', 'American Indian/Alaskan Native', 'Asian/Pacific Islander', 'Other', 'Unknown']
    ...
    return (
        <Card className="text-zinc-300 border-0">
            ...
            <CardContent>
                <div className="mb-4 flex gap-4">
                    <Select onValueChange={(value) => setMetric(value as never)} defaultValue={metric}>
                        <SelectTrigger className="text-zinc-300">
                            <SelectValue className="text-zinc-300" placeholder="Select metric"/>
                        </SelectTrigger>
                        <SelectContent className="bg-zinc-800">
                            <SelectItem className="text-zinc-300" value="% Bail Set">Bail Set %</SelectItem>
                            <SelectItem className="text-zinc-300" value="Average Bail $">Average Bail $</SelectItem>
                            <SelectItem className="text-zinc-300" value="% Released">Released %</SelectItem>
                            <SelectItem className="text-zinc-300" value="% Jailed">Jailed %</SelectItem>
                        </SelectContent>
                    </Select>
                    <Select onValueChange={(value) => setSeverity(value as SeverityLevel)} defaultValue={severity}>
                        <SelectTrigger>
                ...
                <BarChart className="text-red-200" width={600} height={400} data={chartData}
                    ...
                    {races.map((race, index) => (
                        <Bar key={race} dataKey={race} fill={`hsl(${index * 10}, 50%, 50%)`} name={race}/>
                    ))}
                </BarChart>
            </CardContent>
